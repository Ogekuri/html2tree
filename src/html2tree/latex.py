...
3) verifica di compatibilita MathJax
...
LOG = logging.getLogger("html2tree.latex")
...
def _validate_mathjax_compat(latex: str) -> bool:
    ...
    if env_match:
        LOG.debug("MathJax compatibility failed: unsupported environment %s", env_match.group(0))
        return False
    try:
