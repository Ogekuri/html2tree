...
find "$EXAMPLES_DIR" -mindepth 1 -maxdepth 1 -type d -print0 | while IFS= read -r -d '' dir; do
  ...
  if [ -n "${GEMINI_API_KEY:-}" ]; then
    ...
    echo ./html2tree.sh --from-dir "$dir" --to-dir "$out_dir" --verbose --debug | tee -a examples.log
    ./html2tree.sh --from-dir "$dir" --to-dir "$out_dir" --verbose --debug >>examples.log 2>&1 && echo "[OK] on dir $dir" | tee -a examples.log || { rc=$?; echo "[ERROR] (rc=$rc) on dir $dir" | tee -a examples.log; continue; }
  else
