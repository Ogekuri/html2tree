...
if [ -n "${GEMINI_API_KEY:-}" ]; then
    ...
    echo ./http2tree.sh --from-dir html_sample/ --to-dir temp/html_sample_test/ --verbose --debug --post-processing | tee -a html_sample.log
    ./http2tree.sh --from-dir html_sample/ --to-dir temp/html_sample_test/ --verbose --debug --post-processing >>html_sample.log 2>&1 && echo "[OK] on dir html_sample" | tee -a html_sample.log || { rc=$?; echo "[ERROR] (rc=$rc) on dir html_sample" | tee -a html_sample.log; continue; }
else
